import streamlit as st
import pandas as pd
import plotly.graph_objects as go

# --- ページ設定 ---
st.set_page_config(
    page_title="Partnership Health Check",
    page_icon="❤️",
    layout="centered"
)

# --- タイトルと導入 ---
st.title("❤️ Partnership Health Check")
st.caption("Produced by Non-chan / Life Strategist")

st.markdown("""
パートナーシップの状態を**8つの視点**で数値化し、関係の「健康状態」を可視化します。  
直感で **0点（全くそう思わない）〜 10点（非常に強くそう思う）** で回答してください。
""")

st.divider()

# --- 入力フォーム ---
with st.form("check_form"):
    
    st.subheader("Ⅰ. 言語的コミュニケーション (Verbal)")
    st.info("言葉による「意思表示」と「受容」のバランス")
    
    q1 = st.slider("Q1. 【発信：快】ポジティブな要望（好き・したい）を伝えている", 0, 10, 5)
    q2 = st.slider("Q2. 【発信：不快】ネガティブな主張（嫌・やめて）を冷静に伝えている", 0, 10, 5, help="我慢せずに、かつ攻撃的にならずに言えていますか？")
    q3 = st.slider("Q3. 【受信：快】相手の「夢」や「やりたいこと」を積極的に聞いている", 0, 10, 5, help="相手の人生に関心を持てていますか？")
    q4 = st.slider("Q4. 【受信：不快】相手の「NO」や断りを、不機嫌にならず受け止めている", 0, 10, 5)

    st.subheader("Ⅱ. 非言語・身体 (Non-Verbal & Physical)")
    st.info("二人の間に流れる「空気感」と「体温」")
    
    q5 = st.slider("Q5. 【空気感】相手が話しやすい「安心できる空気・表情」を作っている", 0, 10, 5)
    q6 = st.slider("Q6. 【スキンシップ】日常的に手をつなぐ等の肌の温もりを大切にしている", 0, 10, 5)

    st.subheader("Ⅲ. 行動的コミュニケーション (Action)")
    st.info("相手のために「すること」と「ただ居ること」")
    
    q7 = st.slider("Q7. 【能動的貢献(Doing)】家事やサポートなど、相手のための行動をしている", 0, 10, 5)
    q8 = st.slider("Q8. 【受動的共有(Being)】何もしない時間を心地よく共有できている", 0, 10, 5, help="沈黙は怖くないですか？")

    submitted = st.form_submit_button("診断結果を表示する", type="primary")

# --- 結果表示ロジック ---
if submitted:
    st.divider()
    st.header("📊 Diagnosis Result")

    # データの整理
    labels = [
        '発信:快(Q1)', '発信:不快(Q2)', 
        '受信:快(Q3)', '受信:不快(Q4)',
        '空気感(Q5)', 'スキンシップ(Q6)', 
        '貢献(Doing)(Q7)', '共有(Being)(Q8)'
    ]
    values = [q1, q2, q3, q4, q5, q6, q7, q8]
    
    # チャートを閉じる処理
    values_radar = values + [values[0]]
    labels_radar = labels + [labels[0]]

    # レーダーチャート描画
    fig = go.Figure()
    fig.add_trace(go.Scatterpolar(
        r=values_radar,
        theta=labels_radar,
        fill='toself',
        name='Your Score',
        line_color='#FF4B4B',
        fillcolor='rgba(255, 75, 75, 0.3)'
    ))

    fig.update_layout(
        polar=dict(
            radialaxis=dict(
                visible=True,
                range=[0, 10],  # 0-10固定
                tickfont=dict(size=10),
            )
        ),
        showlegend=False,
        margin=dict(l=40, r=40, t=40, b=40)
    )

    st.plotly_chart(fig, use_container_width=True)

    # --- フィードバックコメント ---
    st.subheader("💡 Strategic Advice")

    # 総合スコア計算
    total_score = sum(values)
    avg_score = total_score / 8

    col1, col2 = st.columns(2)
    with col1:
        st.metric("Total Score", f"{total_score} / 80")
    with col2:
        st.metric("Average", f"{avg_score:.1f}")

    # 個別アドバイスロジック
    advice_list = []

    # Q2 (NOを言う力) が低い場合
    if q2 <= 5:
        advice_list.append("⚠️ **「NO」を言う勇気が必要です**\n不快なことを我慢すると、いつか爆発してしまいます。小さな「それは嫌」を伝える練習をしましょう。")

    # Q3 (相手への関心) が低い場合
    if q3 <= 5:
        advice_list.append("⚠️ **相手への「好奇心」が不足気味？**\n自分のことで手一杯かもしれません。「最近どう？」と相手の夢について聞いてみましょう。")

    # バランスが良い場合
    if avg_score >= 8 and (max(values) - min(values) <= 3):
        advice_list.append("🌟 **素晴らしいバランスです！**\nお互いが自立しつつ依存できている、非常に健康的な状態です。")
    
    # Q7が高くてQ8が低い場合 (尽くしすぎ)
    if q7 >= 7 and q8 <= 4:
        advice_list.append("☕ **「何もしない」練習を**\n役に立とうと頑張りすぎていませんか？ ただ横に座ってダラダラする時間も、立派な愛です。")

    # アドバイス表示
    if advice_list:
        for advice in advice_list:
            # 普通のif文に書き換えることで、余計な表示を消す
            if "⚠️" in advice:
                st.warning(advice)
            else:
                st.info(advice)
    else:
        st.success("全体的にバランスが取れています。気になる項目があれば、パートナーと話し合ってみましょう！")